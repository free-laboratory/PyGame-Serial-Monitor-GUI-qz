...
from mujoco_client import send_joint_positions, send_cartesian_delta
import math
...
def recreate_ui_helperfunction(something):
    ...
    current_resolution_string = (
        str(something.options.resolution[0]) + 'x' + str(something.options.resolution[1])
    )

    something.test_drop_down = UIDropDownMenu(
        options_list=['1024x768', '1200x800', '1440x800', '1600x900', '800x600', '600x800'],
        starting_option=current_resolution_string,
        relative_rect=pygame.Rect(
            (int(something.options.resolution[0] * 0.01),
             int(something.options.resolution[1] * 0.01)),
            (int(something.options.resolution[0] / 4),
             int(something.options.resolution[1] / 32))
        ),
    ...
    something.serial_select_dropdown = UIDropDownMenu(
        ...
        relative_rect=pygame.Rect(
            (int(something.options.resolution[0] * 0.99) - int(something.options.resolution[0] / 4),
             int(something.options.resolution[1] * 0.01)),
            (int(something.options.resolution[0] / 4),
             int(something.options.resolution[1] / 32))
        ),
    ...
    something.serial_msg_disp = UITextBox(
        html_text="",
        relative_rect=pygame.Rect(
            (int(something.options.resolution[0] * 0.01 + something.options.resolution[0] / 4),
             int(something.options.resolution[1] * 0.01 + something.options.resolution[1] / 32)),
            (int(something.options.resolution[0] / 2 - something.options.resolution[0] * 0.02),
             int(something.options.resolution[1] * 28 / 32))
        ),
        manager=something.ui_manager
    )
...
def send_ik_step(axis: str, direction: int):
    delta = [0.0] * 6

    if axis == "x":
        delta[0] = _TRANSLATION_STEP * direction
    elif axis == "y":
        delta[1] = _TRANSLATION_STEP * direction
    elif axis == "z":
        delta[2] = _TRANSLATION_STEP * direction
    elif axis == "roll":
        delta[3] = _ROTATION_STEP * direction
    elif axis == "pitch":
        delta[4] = _ROTATION_STEP * direction
    elif axis == "yaw":
        delta[5] = _ROTATION_STEP * direction
    else:
        return

    try:
        send_cartesian_delta(delta)
        print(f"[IK step] axis={axis}, dir={direction}, delta={delta}")
    except Exception as e:
        print(f"[IK send failed] {e}")