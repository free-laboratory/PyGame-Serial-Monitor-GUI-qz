...
class OptionsUIApp:
    def __init__(self, shared_values=None):
        # shared list from multiprocessing.Manager
        ...
        # simple background so blit() is safe
        self.background_surface = pygame.Surface(self.options.resolution)
        self.background_surface.fill((25, 25, 25))
        ...
        # references to UI elements created in recreate_ui
    ...
    def _create_sliders(self):
        ...
        # RIGHT = [box.right + margin .. W - margin]
        ...
        l_slider_x, l_slider_w, l_value_x, l_base_x = side_metrics(
            left_band_start, left_band_end
        )
        r_slider_x, r_slider_w, r_value_x, r_base_x = side_metrics(
        ...
        row_h = max(
            ROW_MIN,
            min(ROW_MAX, (usable_h - (per_col - 1) * PAD_Y) // per_col),
        )
        ...
        UILabel(
            relative_rect=pygame.Rect(
                box_rect.centerx - 50, max(0, box_rect.top - 20), 100, 16
            ),
            text="Controls",
        ...
        for i in range(12):
            ...
            s_w = max(
                SLIDER_MIN_W,
                min(s_w, W - margin - s_x - GAP_S_V - VALUE_W),
            )
            ...
            lbl = UILabel(label_rect, f"S{i+1}", manager=self.ui_manager)
            ...
            val = UILabel(value_rect, str(default), manager=self.ui_manager)

            items.append({"label": lbl, "slider": sld, "value": val})

        self._slider_rows = items

        # initialize shared values from defaults
        if self.shared_values is not None:
            for idx, row in enumerate(self._slider_rows):
                if idx < len(self.shared_values):
                    self.shared_values[idx] = float(default)
    ...
    def check_resolution_changed(self):
        ...
        if (
            resolution_width != self.options.resolution[0]
    ...
    def process_events(self):
        ...
        for event in pygame.event.get():
            ...
            # let pygame_gui handle all UI widgets
            self.ui_manager.process_events(event)

            # ---- handle slider moves (pygame_gui UIHorizontalSlider) ----
            is_slider_event = (
                event.type == pygame.USEREVENT
                and getattr(event, "user_type", None)
            ...
            if is_slider_event and hasattr(self, "_slider_rows"):
                for idx, row in enumerate(self._slider_rows):
                    if event.ui_element is row["slider"]:
                        ...
                        # track currently active slider
                        self._active_slider_idx = idx
                        self._active_slider_val = val

                        # update shared_values immediately
                        if (
                            self.shared_values is not None
            ...
            elif event.type == pygame.MOUSEBUTTONUP and hasattr(
                self, "_active_slider_idx"
            ):
                idx = self._active_slider_idx
                val = getattr(self, "_active_slider_val", None)
                if val is not None:
                    on_slider_changed(idx, val)
                # clear state
                self._active_slider_idx = None
            ...
            if event.type == pygame_gui.UI_BUTTON_PRESSED:
                    ...
                    try:
                        print(
                            "--Connecting to {}".format(
                                self.serial_select_dropdown.selected_option[0]
                            )
                        )
                        conn_baud = int(self.serial_baudrate_textbox.get_text())
                        gvar_ctrl.mcu_serial_object = serial.Serial(
                            port=self.serial_select_dropdown.selected_option[0],
                            baudrate=conn_baud,
                            timeout=0.1,
                        )
                    except Exception:
                ...
                if event.ui_element == self.serial_refresh_button:
                    try:
                        ...
                        self.serial_select_dropdown.options_list = [
                        ...
                        print("i don't know why but refreshing failed")
    ...
    def run(self):
        ...
        global context  # if you use this elsewhere
        ...
        while self.running:
            ...
            # draw background + full UI (including sliders)
